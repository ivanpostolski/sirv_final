...
SIRStates = enum(S='Susceptible', I='Infected',  R='Recovered',V='Vaccinated')
ENVProps = enum(DECAY='decay_rate', AGENT_STATES='log data'\
    , QUARANTINE_CONDITION='If above threshold, agents do quarantine.',\
         AVAILABLE_VACCINES='If there is available vaccines')
...
class AgentState(object):
    def __init__(self, model, name, id, state, env):
        """TODO: to be defined1. """
        self._name = name
        ...
        self._to_vaccinate = False
        self._emergence = False
        self.neighbors = 0

        self.free_deg = env.degree_gen(self) if id!=0 else 0

        self.deg = int(self.free_deg)
        self.model = model
    ...
    def set_infection_values(self): 
        ...
        if np.array(neighbors_states).size >  0 and sum([x==SIRStates.S for x in list(neighbors_states)]) > 0:
            susceptible_neighbors = sum([x==SIRStates.S for x in list(neighbors_states)])
    ...
    @property
    def to_vaccinate(self):
        """I'm the 'heading' property."""
        return self._to_vaccinate

    @to_recover.setter
    def to_vaccinate(self, to_vaccinate):
        self._to_vaccinate = to_vaccinate
...
class Agent(AtomicDEVS):
    ...
    def __init__(self, env, name=None, id=None):
        """
        ...
        self.state = AgentState(self, self.name, id, state,env)
    ...
    def intTransition(self):
        ...
        if self.state.to_recover:
            ...
            pass
        elif self.state.state == SIRStates.S and self.state.ta > 0:
            if (self.parent.getContextInformation(ENVProps.AVAILABLE_VACCINES)):
                self.state.state = SIRStates.V
                self.state.model_transition = True
                self.state.share = True
            else: 
                self.state.ta = INFINITY
    ...
    def modelTransition(self, state):
        ...
        if self.state.model_transition and self.state.state == SIRStates.V:
            state["V"] = self.state
            self.state.model_transition = False
            return True
    ...
    def timeAdvance(self):
        ...
        if self.state.share:
            ...
            # and not self.state.to_recover:
            self.state.set_infection_values()
        elif self.state.state in (SIRStates.R, SIRStates.V):
            self.state.ta = INFINITY
        elif self.state.state == SIRStates.S and (self.state.ta == 0 or self.state.ta == INFINITY):
            # vaccination strategy
            self.state.ta = self.parent.vaccine_strategy(self.state,self.parent,Parameters)
...
class Environment(CoupledDEVS):
    def __init__(self, degree_gen, vaccine_props, vaccine_strategy,name=None):
        """
        ...
        self.nodes_free_deg = {}
        self.degree_gen = degree_gen["generator"] 
        Parameters.PHI = degree_gen["mean"] 
        ...
        self.vaccine_scale = vaccine_props["vaccine_scale"]
        self.vaccines_available = vaccine_props["vaccine_limit"]
        self.vaccine_strategy = vaccine_strategy["fun"]
    ...
    def create_topology(self):
        ...
        self.agents = [Agent(self,name="agent %d" % i, id=i) for i in range(G.number_of_nodes())]
    ...
    def getContextInformation(self, property, *args, **kwargs):
        ...
        if(property == ENVProps.AVAILABLE_VACCINES):
            return self.vaccines_available > 0
    ...
    def modelTransition(self, state): 
        ...
        if "V" in state:
            self.vaccines_available = self.vaccines_available - 1
            del state["V"]
            return False
